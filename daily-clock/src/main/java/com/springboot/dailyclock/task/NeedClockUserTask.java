...
//    @Scheduled(cron="0 0 5 * * *")
        ...
        for (UserAccountModel model : userAccountModelList) {
            ...
            if ("".equals(model.getUseBalance0())) { // 当前打卡周期结束，今日无需打卡
                ...
                if (orderP.getDays() == 1) { // 如果昨天充值，则今天可以打卡
                    commonClock(no, model.getOpenid(), model.getUseBalance0(), map);
                } else {
                    // 再判断昨日是否正常打卡
     ...
     *          缓存的数据
     */
    private void commonClock(String no, String openid, String balance,  Map<String, Object> map) {
        ...
        needClockUserModel.setUseBalance(balance);
